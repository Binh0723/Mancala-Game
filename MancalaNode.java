...
scoring pit.  Thus a simple measure of utility would be (s - s).
	 ...
	 * will draw. Index -1 will return a standard (unfair) Mancala initial board state.
	 * Index 0 will return a random FairKalah initial board state.  
	 * Indices 1-254 will return specific FairKalah initial board states. 
	 * Indices beyond those bounds will throw an IllegalArgumentException.
	 * @param fairStateIndex either a valid state index 1-254 or an implicit request (0) for a 
	 *        random FairKalah initial state, or -1 indicating a standard (unfair) 
	 *        Mancala initial state.
	 */
	public MancalaNode(int fairStateIndex) {
		this();
		if (fairStateIndex != -1) {
			if (fairStateIndex < -1 || fairStateIndex > fairkalahStates.length)
				throw new IllegalArgumentException(String.format("MancalaNode(int): int should be in range -1 - %d, was %d", fairkalahStates.length, fairStateIndex));
			else if (fairStateIndex == 0)
				fairStateIndex = (int) (fairkalahStates.length * Math.random()) + 1;
			state = fairkalahStates[fairStateIndex - 1].clone();
		}
	}
	...
	public void makeMove(int move) {
		int position = move;
		prevMove = move;
		int scorePit = (player == MAX) ? MAX_SCORE_PIT : MIN_SCORE_PIT;
		int oppositePit = MIN_SCORE_PIT - position - 1;

		// Check for illegal move
        if (position < scorePit - PLAY_PITS || position >= scorePit 
        		|| state[position] == 0)
            throw new IllegalArgumentException("makeMove: Illegal move " + move);
